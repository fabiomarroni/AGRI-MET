...
option_list = list(
  ...
  make_option(c("-I", "--includesmall"), type="logical", default="TRUE",
...
# smalldat<-smalldat[,grep("Number_",names(smalldat),invert=T)]
# names(smalldat)<-gsub("Percent_","",names(smalldat))
...
#FIN QUI. Continuare con le analisi per capire cosa sta succedendo. La tabella finres contiene tutti i risultati.
#cortese<-finres[finres$Var=="cort_gva_merged",]
finres$labels_RT<-NA
...
ROCit_obj_a_both <- rocit(score=finres$aRNA,class=finres$labels)
ROCit_obj_b_both <- rocit(score=finres$bRNA,class=finres$labels)
ROCit_obj_small_both <- rocit(score=finres$smallRNA,class=finres$labels)
ROCit_obj_consmall_both <- rocit(score=finres$consmallRNA,class=finres$labels)
plot.rocit(ROCit_obj_a_both,YIndex=F,add.x.lab=TRUE,add.y.lab=TRUE,col=col.list,legend=TRUE)
mtext("Both",side=2,cex=0.6,line=2)
mtext(bquote("total" ~  RNA^a),side=1,cex=0.6,line=2)
plot.rocit(ROCit_obj_b_both,YIndex=F,add.x.lab=TRUE,col=col.list,legend=TRUE)
mtext(bquote("total" ~ RNA^b),side=1,cex=0.6,line=2)
if(includesmall) 
{
	plot.rocit(ROCit_obj_small_both,YIndex=F,add.x.lab=TRUE,col=col.list,legend=TRUE)
	mtext(bquote("small" ~ RNA^a),side=1,cex=0.6,line=2)
	plot.rocit(ROCit_obj_consmall_both,YIndex=F,add.x.lab=TRUE,col=col.list,legend=TRUE)
...
opt_youden_a_both<-ROCit_obj_a_both$Cutoff[which.max(ROCit_obj_a_both$TPR-ROCit_obj_a_both$FPR)]
opt_TPR_a_both<-ROCit_obj_a_both$TPR[which.max(ROCit_obj_a_both$TPR-ROCit_obj_a_both$FPR)]
opt_FPR_a_both<-ROCit_obj_a_both$FPR[which.max(ROCit_obj_a_both$TPR-ROCit_obj_a_both$FPR)]
...
opt_youden_b_both<-ROCit_obj_b_both$Cutoff[which.max(ROCit_obj_b_both$TPR-ROCit_obj_b_both$FPR)]
opt_TPR_b_both<-ROCit_obj_b_both$TPR[which.max(ROCit_obj_b_both$TPR-ROCit_obj_b_both$FPR)]
opt_FPR_b_both<-ROCit_obj_b_both$FPR[which.max(ROCit_obj_b_both$TPR-ROCit_obj_b_both$FPR)]
...
opt_youden_small_both<-ROCit_obj_small_both$Cutoff[which.max(ROCit_obj_small_both$TPR-ROCit_obj_small_both$FPR)]
opt_TPR_small_both<-ROCit_obj_small_both$TPR[which.max(ROCit_obj_small_both$TPR-ROCit_obj_small_both$FPR)]
opt_FPR_small_both<-ROCit_obj_small_both$FPR[which.max(ROCit_obj_small_both$TPR-ROCit_obj_small_both$FPR)]
...
opt_youden_consmall_both<-ROCit_obj_consmall_both$Cutoff[which.max(ROCit_obj_consmall_both$TPR-ROCit_obj_consmall_both$FPR)]
opt_TPR_consmall_both<-ROCit_obj_consmall_both$TPR[which.max(ROCit_obj_consmall_both$TPR-ROCit_obj_consmall_both$FPR)]
opt_FPR_consmall_both<-ROCit_obj_consmall_both$FPR[which.max(ROCit_obj_consmall_both$TPR-ROCit_obj_consmall_both$FPR)]
...
rres<-matrix(c(opt_youden_a_ELISA,ROCit_obj_a_ELISA$AUC,opt_TPR_a_ELISA,opt_FPR_a_ELISA,length(ROCit_obj_a_ELISA$pos_D),length(ROCit_obj_a_ELISA$neg_D),
               ...
               opt_youden_a_both,ROCit_obj_a_both$AUC,opt_TPR_a_both,opt_FPR_a_both,length(ROCit_obj_a_both$pos_D),length(ROCit_obj_a_both$neg_D),
               opt_youden_b_both,ROCit_obj_b_both$AUC,opt_TPR_b_both,opt_FPR_b_both,length(ROCit_obj_b_both$pos_D),length(ROCit_obj_b_both$neg_D),
               opt_youden_consmall_both,ROCit_obj_consmall_both$AUC,opt_TPR_consmall_both,opt_FPR_consmall_both,length(ROCit_obj_consmall_both$pos_D),length(ROCit_obj_consmall_both$neg_D),
               opt_youden_small_both,ROCit_obj_small_both$AUC,opt_TPR_small_both,opt_FPR_small_both,length(ROCit_obj_small_both$pos_D),length(ROCit_obj_small_both$neg_D)),
      ncol=6,byrow=T,dimnames=list(c("aRNA_ELISA","bRNA_ELISA","contig_small_ELISA","small_ELISA","aRNA_RT","bRNA_RT","contig_small_RT","small_RT","aRNA_both","bRNA_both","contig_small_both","small_both"),c("Youden","AUC","TPR","FPR","Positives","Negatives")))
